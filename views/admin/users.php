    ...
    <section class="admin-panel">
        ...
        <form action="/admin/usuarios/salvar" method="post" class="admin-form">
            ...
            <div class="field-grid">
                ...
                <label>Nível de acesso<select name="role"><option value="auditor">Auditor — permissões configuráveis</option><option value="administrator">Administrador — acesso completo</option></select></label>
            </div>
            ...
            <?php foreach($users as $row): ?>
                <details class="user-admin-item">
                    ...
                    <form action="/admin/usuarios/salvar" method="post" class="admin-form">
                        ...
                        <div class="field-grid"><label>Nome<input name="name" value="<?= $h($row['name']) ?>" required></label><label>E-mail<input type="email" name="email" value="<?= $h($row['email']) ?>" required></label><label>Nova senha <span>deixe vazio para manter a atual</span><input type="password" name="password" minlength="12" autocomplete="new-password"></label><label>Nível de acesso<select name="role"><option value="auditor" <?= $row['role']==='auditor'?'selected':'' ?>>Auditor</option><option value="administrator" <?= $row['role']==='administrator'?'selected':'' ?>>Administrador</option></select></label></div>
